...
keywords	iparam jparam seed ieach jeach
arguments:
  ...
  Fix(lmp, narg, arg)
{
  ...
  //set the type of bonds (two/pathway, slip)
...
void FixCatchBond::post_integrate()
{
  ...
  if (type_flag == 1) {
    for (i = 0; i < nlocal; i++){
      if (partner_b[i]) {
        vec_fraction_b[i] = TwoPathway(sqrt(distsq_b[i]));
    ...
    for (i = 0; i < nlocal; i++){
      if (partner_b[i]) {
        vec_fraction_b[i] = Slip(sqrt(distsq_b[i]));
        probability_b[i] = random->uniform();
      }
      if (partner_c[i]) {
        vec_fraction_c[i] = bond_create(sqrt(distsq_c[i]));
    ...
    for (i = 0; i < nlocal; i++){
      if (partner_b[i]) {
        vec_fraction_b[i] = Flex(sqrt(distsq_b[i]));
        probability_b[i] = random->uniform();
      }
      if (partner_c[i]) {
        vec_fraction_c[i] = bond_create(sqrt(distsq_c[i]));
