...
#include "stdlib.h"
#include "string.h"
#include "pair_fluidvismembrane.h"
...
static const char cite_pair_fluidvismembrane[] =
  "pair fluidvismembrane command:\n\n";

/* ---------------------------------------------------------------------- */

Pairfluidvismembrane::Pairfluidvismembrane(LAMMPS *lmp) : Pair(lmp)
{
  if (lmp->citeme) lmp->citeme->add(cite_pair_fluidvismembrane);
...
Pairfluidvismembrane::~Pairfluidvismembrane()
{
  if (allocated) {
...
void Pairfluidvismembrane::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        one_eng = fluidvismembrane_analytic(i,j,a1,a2,r12,v12,rsq,fforce,ttor,rtor);
...
void Pairfluidvismembrane::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void Pairfluidvismembrane::settings(int narg, char **arg)
{
...
void Pairfluidvismembrane::coeff(int narg, char **arg)
{
  if (narg < 9 || narg > 9)
    error->all(FLERR,"Incorrect args for pair fluidvismembrane coefficients");
...
void Pairfluidvismembrane::init_style()
{
  avec = (AtomVecEllipsoid *) atom->style_match("ellipsoid");
  if (!avec) error->all(FLERR,"Pair fluidvismembrane requires atom style ellipsoid");

  if (comm->ghost_velocity == 0)
    error->all(FLERR,"Pair fluidvismembrane requires ghost atoms store velocity");

  // if newton off, forces between atoms ij will be double computed
  // using different random numbers

  if (force->newton_pair == 0 && comm->me == 0) error->warning(FLERR,
      "Pair fluidvismembrane needs newton pair on for momentum conservation");
...
double Pairfluidvismembrane::init_one(int i, int j)
{
...
void Pairfluidvismembrane::write_restart(FILE *fp)
{
...
void Pairfluidvismembrane::read_restart(FILE *fp)
{
...
void Pairfluidvismembrane::write_restart_settings(FILE *fp)
{
...
void Pairfluidvismembrane::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void Pairfluidvismembrane::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    fprintf(fp,"%d %g %g %g %g %g %g\n",i,
            epsilon[i][i],sigma[i][i],cut[i][i],zeta[i][i],mu[i][i], beta[i][i], gamma[i][i]);
}
...
void Pairfluidvismembrane::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %g %g %g %g %g %g\n",i,j,
              epsilon[i][i],sigma[i][i],cut[i][j],zeta[i][j],mu[i][j], beta[i][j], gamma[i][i]);
}
...
double Pairfluidvismembrane::fluidvismembrane_analytic(const int i,const int j,double a1[3][3],
...
void *Pairfluidvismembrane::extract(const char *str, int &dim)
{
  dim = 2;
  if (strcmp(str,"epsilon") == 0) return (void *) epsilon;
  if (strcmp(str,"sigma") == 0) return (void *) sigma;
  if (strcmp(str,"cut") == 0) return (void *) cut;
  if (strcmp(str,"zeta") == 0) return (void *) zeta;
