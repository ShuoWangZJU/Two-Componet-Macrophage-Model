...
Neighbor::Neighbor(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  maxbond = 0;
  bondlist = NULL;
  bondlist_length = NULL;
  maxangle = 0;
  anglelist = NULL;
  anglelist_area = NULL;
  maxdihedral = 0;
  dihedrallist = NULL;
  dihedrallist_angle = NULL;
  maximproper = 0;
  improperlist = NULL;
...
void Neighbor::init()
{
  ...
  if (atom->molecular && atom->nbonds && maxbond == 0) {
    ...
    if(atom->individual)
	bondlist_length = (double *) memory->smalloc(maxbond*sizeof(double),"bondlist_length");
  }
  ...
  if (atom->molecular && atom->nangles && maxangle == 0) {
    ...
    if(atom->individual)
      anglelist_area = (double *) memory->smalloc(maxangle*sizeof(double),"anglelist_area");
  }
  ...
  if (atom->molecular && atom->ndihedrals && maxdihedral == 0) {
   ...
   if(atom->individual)
      dihedrallist_angle = (double *) memory->smalloc(maxdihedral*sizeof(double),"dihedallist_angle"); 
  }
...
bigint Neighbor::memory_usage()
{
  ...
  bytes += memory->usage(bondlist_length, maxbond);
  bytes += memory->usage(anglelist,maxangle,4);
  bytes += memory->usage(anglelist_area, maxangle);
  bytes += memory->usage(dihedrallist,maxdihedral,5);
  bytes += memory->usage(dihedrallist_angle,maxdihedral);
