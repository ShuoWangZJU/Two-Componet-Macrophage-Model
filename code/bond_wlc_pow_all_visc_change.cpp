...
BondWLC_POW_ALL_VISC_CHANGE::BondWLC_POW_ALL_VISC_CHANGE(LAMMPS *lmp) : Bond(lmp) 
{
  random = NULL;
  mtrand = NULL;
}
...
BondWLC_POW_ALL_VISC_CHANGE::~BondWLC_POW_ALL_VISC_CHANGE()
{
  if (allocated) {
    ...
    memory->destroy(init_count);
    memory->destroy(igroup_count);
    memory->destroy(change_ratio);
  }
...
void BondWLC_POW_ALL_VISC_CHANGE::compute(int eflag, int vflag)
{
  int i,i1,i2,n,type,k,l;
  ...
  for (i = 1; i <= atom->nbondtypes; i++){
	if (igroup_count[i] != -1 && init_count[i] != 0) {
	  group_num = group->count(igroup_count[i]);
	  change_ratio[i] = sqrt(double(group_num)/init_count[i]);
	  if (comm->me == 0 && update->ntimestep % 100 == 0) fprintf(stderr, "In pair change when timestep = %d: bondtype = %d, change_ratio = %f\n", update->ntimestep, i , change_ratio[i]);
	}
  }
...
void BondWLC_POW_ALL_VISC_CHANGE::allocate()
{
  allocated = 1;
  ...
  memory->create(init_count,n+1,"bond:init_count");
  memory->create(igroup_count,n+1,"bond:igroup_count");
  memory->create(change_ratio,n+1,"bond:change_ratio");
...
void BondWLC_POW_ALL_VISC_CHANGE::coeff(int narg, char **arg)
{
  if (narg != 10) error->all(FLERR,"Incorrect args in bond_coeff command");
  ...
  int init_count_one = force->inumeric(FLERR,arg[8]);
  int igroup_count_one = group->find(arg[9]);
  if (igroup_count_one == -1) error->all(FLERR,"Could not find count group ID in bond_coeff command");
  ...
  for (int i = ilo; i <= ihi; i++) {
	...
	init_count[i] = init_count_one;
	igroup_count[i] = igroup_count_one;
    setflag[i] = 1;
    count++;
  }
...
void BondWLC_POW_ALL_VISC_CHANGE::init_style()
{
  ...
  for (int i = 1; i <= atom->nbondtypes; i++){
	...
	if (igroup_count[i] != -1 && init_count[i] != 0) {
	  group_num = group->count(igroup_count[i]);
	  change_ratio[i] = sqrt(group_num/init_count[i]);
	}
  } 
}
...
double BondWLC_POW_ALL_VISC_CHANGE::equilibrium_distance(int i)
{
  return r0[i];
}
...
void BondWLC_POW_ALL_VISC_CHANGE::write_restart(FILE *fp)
{
  ...
  fwrite(&init_count[1],sizeof(int),atom->nbondtypes,fp);
  fwrite(&igroup_count[1],sizeof(int),atom->nbondtypes,fp);
}
...
void BondWLC_POW_ALL_VISC_CHANGE::read_restart(FILE *fp)
{
  allocate();
  ...
  if (comm->me == 0) {
    ...
    fread(&init_count[1],sizeof(int),atom->nbondtypes,fp); 
    fread(&igroup_count[1],sizeof(int),atom->nbondtypes,fp); 
  }
  ...
  MPI_Bcast(&init_count[1],atom->nbondtypes,MPI_INT,0,world);
  MPI_Bcast(&igroup_count[1],atom->nbondtypes,MPI_INT,0,world);
...
void BondWLC_POW_ALL_VISC_CHANGE::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nbondtypes; i++)
    fprintf(fp,"%d %g %g %g %g %g %g %g\n",i,temp[i],r0[i],mu_targ[i],qp[i],gamc[i],gamt[i],spring_scale[i],init_count[i],igroup_count[i]);
}

/* ----------------------------------------------------------------------*/

double BondWLC_POW_ALL_VISC_CHANGE::single(int type, double rsq, int i, int j,
                        double &fforce)
{
  char warning[128];
...
void BondWLC_POW_ALL_VISC_CHANGE::generate_wrr()
{
  int i;
  double ww[3][3];
