...
void Atom::deallocate_topology()
{
  ...
  if (individual){
    ...
    atom->bond_length = atom->angle_area = atom->dihedral_angle = NULL;
  }
}
...
void Atom::set_moment(const char *str, int type_offset)
{
  if (moment == NULL) error->all(FLERR,"Cannot set moment for this atom style");

  int itype;
  double moment_one;
  int n = sscanf(str,"%d %lg",&itype,&moment_one);
  if (n != 2) error->all(FLERR,"Invalid moment line in data file");
  itype += type_offset;

  if (itype < 1 || itype > ntypes) error->all(FLERR,"Invalid type for moment set");

  moment[itype] = moment_one;
  moment_setflag[itype] = 1;

  if (moment[itype] <= 0.0) error->all(FLERR,"Invalid moment value");
}
...
void Atom::set_moment(int itype, double value)
{
  if (moment == NULL) error->all(FLERR,"Cannot set moment for this atom style");
  if (itype < 1 || itype > ntypes) error->all(FLERR,"Invalid type for moment set");
...
void Atom::set_moment(int narg, char **arg)
{
  if (moment == NULL) error->all(FLERR,"Cannot set moment for this atom style");

  int lo,hi;
  force->bounds(arg[0],ntypes,lo,hi);
  if (lo < 1 || hi > ntypes) error->all(FLERR,"Invalid type for moment set");
...
/* ----------------------------------------------------------------------
   check that all moments have been set
------------------------------------------------------------------------- */

void Atom::check_moment()
{
  if (moment == NULL) return;
  for (int itype = 1; itype <= ntypes; itype++)
    if (moment_setflag[itype] == 0) 
      error->all(FLERR,"Not all per-type moments are set");
}
