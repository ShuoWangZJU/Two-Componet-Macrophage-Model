...
FixBcRegion::FixBcRegion(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
	if (narg < 7) error->all(FLERR,"Illegal fix bc/region command");
	iregion = domain->find_region(arg[3]);
	BX0 = force->numeric(FLERR,arg[4]);
	BY0 = force->numeric(FLERR,arg[5]);
	BZ0 = force->numeric(FLERR,arg[6]);
	if (iregion == -1) error->all(FLERR,"Fix bc/region command region ID does not exist");
...
void FixBcRegion::init()
{
	int i,j;
	// set constants that depend on fix mvv

	for (i = 0; i < modify->nfix; i++)
		if (strcmp(modify->fix[i]->style,"bc/region") == 0) break;
	if (i < modify->nfix)
	{
...
void FixBcRegion::setup(int vflag)
{
...
void FixBcRegion::initial_integrate(int vflag)
{
	...
	for (int i = 0; i < nlocal; i++) 
	{
		if (mask[i] & groupbit) 
		{
			...
			if (domain->regions[iregion]->match(x0, y0, z0))
			{
				...
				// fprintf(stderr, "In fix bc/region when time = %d, atom %d entered the region, x[i][0] = %f, x[i][1] = %f, x[i][2] = %f, px[0] = %f, px[1] = %f, px[2] = %f\n", update->ntimestep, atom->tag[i], x[i][0], x[i][1], x[i][2], px[0], px[1], px[2]);
				...
				//goto fix mvv and update prev step data 
				/* ((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][0][0] =
					-((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][0][0];
				((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][0][1] =
					-((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][0][1];
				((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][0][2] =
					-((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][0][2];
				((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][1][0] =
					-((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][1][0];
				((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][1][1] =
					-((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][1][1];
				((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][1][2] =
					-((FixMvvDPD *)(modify->fix[ifixmvv]))->f_level[i][1][2]; */
				vest[i][0] = -vest[i][0];
				vest[i][1] = -vest[i][1];
				vest[i][2] = -vest[i][2];
...
void FixBcRegion::post_force(int vflag)
{
...
void FixBcRegion::grow_arrays(int nmax)
{
...
void FixBcRegion::copy_arrays(int i, int j)
{
	//shear[j][0] = shear[i][0];
	//shear[j][1] = shear[i][1];
	//shear[j][2] = shear[i][2];
...
int FixBcRegion::memory_usage(int nmax)
{
	int bytes = 0;
...
int FixBcRegion::pack_exchange(int i, double *buf)
{
	int m = 0;
	//buf[m++] = shear[i][0];
	//buf[m++] = shear[i][1];
...
int FixBcRegion::unpack_exchange(int nlocal, double *buf)
{
	int m = 0;
...
int FixBcRegion::pack_restart(int i, double *buf)
{
	int m = 0;
	//buf[m++] = 4;
	//buf[m++] = shear[i][0];
...
void FixBcRegion::unpack_restart(int nlocal, int nth)
{
...
int FixBcRegion::maxsize_restart()
{
	return 0;
}
...
int FixBcRegion::size_restart(int nlocal)
{
	return 0;
}
