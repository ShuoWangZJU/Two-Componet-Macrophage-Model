...
FixStyle(protrusiveskeleton,FixProtrusiveSkeleton)

#else

#ifndef LMP_FIX_PROTRUSIVESKELETON_H
#define LMP_FIX_PROTRUSIVESKELETON_H

#include "fix.h"

namespace LAMMPS_NS {

class FixProtrusiveSkeleton : public Fix {
 public:
  FixProtrusiveSkeleton(class LAMMPS *, int, char **);
  ~FixProtrusiveSkeleton();
  int setmask();
  void init();
  void setup(int);
  void min_setup(int);
