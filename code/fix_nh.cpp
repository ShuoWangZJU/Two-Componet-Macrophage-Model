  ...
  if (rmass) {
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        v[i][1] += dtfm*f[i][1];
        v[i][2] += dtfm*f[i][2];
//if(update->ntimestep>19000 && (atom->tag[i] == 34333 || atom->tag[i] == 34368)) fprintf(stderr, "In fix_nh nve_v() part01 at timestep %d at proc %d: i = %d, atom->tag[i] = %d, v[i][0] = %f, v[i][1] = %f, v[i][2] = %f, f[i][0] = %f, f[i][1] = %f, f[i][2] = %f\n", update->ntimestep, comm->me, i, atom->tag[i], v[i][0], v[i][1], v[i][2], f[i][0], f[i][1], f[i][2]);
      }
    }
  } else {
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        v[i][1] += dtfm*f[i][1];
        v[i][2] += dtfm*f[i][2];
//if(update->ntimestep>19000 && (atom->tag[i] == 34333 || atom->tag[i] == 34368)) fprintf(stderr, "In fix_nh nve_v() part02 at timestep %d at proc %d: i = %d, atom->tag[i] = %d, v[i][0] = %f, v[i][1] = %f, v[i][2] = %f, f[i][0] = %f, f[i][1] = %f, f[i][2] = %f\n", update->ntimestep, comm->me, i, atom->tag[i], v[i][0], v[i][1], v[i][2], f[i][0], f[i][1], f[i][2]);
      }
    }
  }
}
...
void FixNH::nve_x()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      x[i][1] += dtv * v[i][1];
      x[i][2] += dtv * v[i][2];
//if(update->ntimestep>19000 && (atom->tag[i] == 34333 || atom->tag[i] == 34368)) fprintf(stderr, "In fix_nh nve_x() at timestep %d at proc %d: i = %d, atom->tag[i] = %d, v[i][0] = %f, v[i][1] = %f, v[i][2] = %f, x[i][0] = %f, x[i][1] = %f, x[i][2] = %f\n", update->ntimestep, comm->me, i, atom->tag[i], v[i][0], v[i][1], v[i][2], x[i][0], x[i][1], x[i][2]);
