...
FixStyle(fluidmem/area/volume,FixFluidMemAreaVolume)

#else

#ifndef LMP_FIX_FLUIDMEM_AREA_VOLUME_H
#define LMP_FIX_FLUIDMEM_AREA_VOLUME_H

#include "fix.h"

namespace LAMMPS_NS {

class FixFluidMemAreaVolume : public Fix {
 public:
  FixFluidMemAreaVolume(class LAMMPS *, int, char **);
  ~FixFluidMemAreaVolume();
  int setmask();
  void init();
  void pre_neighbor();
  void pre_force(int);
 ...
 private:
  int N_near, neigh_flag, in_plane_num, average_area_flag, average_volume_flag;
  double d_max, ka, kv, a0, v0, a0_ave, v0_ave;
  int nlocal_group;
  int *group_atom_ID;
  int **nearby_atom_ID;
  int *nearby_atom_num;
