...
#include "stdlib.h"
#include "string.h"
#include "pair_fluidmembrane_range.h"
...
static const char cite_pair_fluidmembrane_range[] =
  "pair fluidmembrane range command:\n\n";

/* ---------------------------------------------------------------------- */

Pairfluidmembranerange::Pairfluidmembranerange(LAMMPS *lmp) : Pair(lmp)
{
  if (lmp->citeme) lmp->citeme->add(cite_pair_fluidmembrane_range);
...
Pairfluidmembranerange::~Pairfluidmembranerange()
{
  if (allocated) {
...
void Pairfluidmembranerange::compute(int eflag, int vflag)
{
...
void Pairfluidmembranerange::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void Pairfluidmembranerange::settings(int narg, char **arg)
{
...
void Pairfluidmembranerange::coeff(int narg, char **arg)
{
  if (narg < 8 || narg > 8)
...
void Pairfluidmembranerange::init_style()
{
...
double Pairfluidmembranerange::init_one(int i, int j)
{
...
void Pairfluidmembranerange::write_restart(FILE *fp)
{
...
void Pairfluidmembranerange::read_restart(FILE *fp)
{
...
void Pairfluidmembranerange::write_restart_settings(FILE *fp)
{
...
void Pairfluidmembranerange::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void Pairfluidmembranerange::write_data(FILE *fp)
{
...
void Pairfluidmembranerange::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double Pairfluidmembranerange::fluidmembrane_analytic(const int i,const int j,double a1[3][3],
