...
FixStyle(filopodia/random,FixFilopodiaRandom)

#else

#ifndef LMP_FIX_FILOPODIA_RANDOM_H
#define LMP_FIX_FILOPODIA_RANDOM_H

#include "fix.h"

namespace LAMMPS_NS {

class FixFilopodiaRandom : public Fix {
 public:
  FixFilopodiaRandom(class LAMMPS *, int, char **);
  ~FixFilopodiaRandom();
  int setmask();
  void init();
