...
FixActiveProtrusion::FixActiveProtrusion(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg != 9 && narg != 11 && narg != 12 && narg != 14) error->all(FLERR,"Illegal fix ActiveProtrusion command");
  ...
  if (igroup_mol == -1) error->all(FLERR,"Could not find molecule group ID for fix ActiveProtrusion command");
  ...
  inactive_type = force->inumeric(FLERR,arg[6]);
  active_type = force->inumeric(FLERR,arg[7]);
  r_active = force->numeric(FLERR,arg[8]);
  ...
  if (narg == 9){
	...
	if (strcmp(arg[9],"around") == 0) {
      around_flag= 1;
      Raround = force->numeric(FLERR,arg[10]);
	}
	else error->all(FLERR,"Illegal fix ActiveProtrusion command");
  }
  else if (narg == 12){
	if (strcmp(arg[9],"bystep") == 0) {
      bystep_flag= 1;
      Ndiv = force->inumeric(FLERR,arg[10]);
      finalstep = force->inumeric(FLERR,arg[11]);
      if (finalstep % Ndiv) error->all(FLERR,"The timestep is not an integer multiple of the increase times for  the gradual-mode fix ActiveProtrusion command");
	...
	if (strcmp(arg[9],"bystep") == 0 && strcmp(arg[12],"around") == 0) {
      bystep_flag= 1;
      Ndiv = force->inumeric(FLERR,arg[10]);
      finalstep = force->inumeric(FLERR,arg[11]);
      if (finalstep % Ndiv) error->all(FLERR,"The timestep is not an integer multiple of the increase times for  the gradual-mode fix ActiveProtrusion command");
	  ...
	  around_flag= 1;
	  Raround = force->numeric(FLERR,arg[13]);
	}
	else error->all(FLERR,"Illegal fix ActiveProtrusion command");
  }
...
FixActiveProtrusion::~FixActiveProtrusion()
{
  //memory->destroy(ActiveProtrusion;);
}

/* ---------------------------------------------------------------------- */

int FixActiveProtrusion::setmask()
{
...
void FixActiveProtrusion::setup(int vflag)
{
...
void FixActiveProtrusion::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixActiveProtrusion::post_force(int vflag)
{
  int step;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // reset all mem atoms to be inactive
  for (m=0; m<nlocal; m++){
    if ((mask[m] & groupbit) && (type[m] == inactive_type || type[m] == active_type)){
	  mask[m] &= inversebit_active;
	  type[m] = inactive_type;
	}
  }
...
void FixActiveProtrusion::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixActiveProtrusion::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixActiveProtrusion::compute_scalar()
{
...
double FixActiveProtrusion::compute_vector(int n)
{
  // only sum across procs one time
...
double FixActiveProtrusion::memory_usage()
{
  double bytes = 0.0;
