...
FixSkeleBreak::FixSkeleBreak(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg != 8 && narg != 10 && narg != 12 && narg != 14 && narg != 16) error->all(FLERR,"Illegal fix SkeleBreak command");

  MPI_Comm_rank(world,&me);

  nevery = force->inumeric(FLERR,arg[3]);
  if (nevery <= 0) error->all(FLERR,"Illegal fix SkeleBreak command");
  ...
  if (igroup_target == -1) error->all(FLERR,"Could not find molecule group ID for fix SkeleBreak command");
  groupbit_target = group->bitmask[igroup_target];
  
  r_break = force->numeric(FLERR,arg[6]);
  if (r_break < 0.0) error->all(FLERR,"Illegal fix SkeleBreak command");
  ...
  if (narg == 10 && strcmp(arg[8],"excluded_group") == 0) {
	group_excluded_flag = 1;
	igroup_excluded = group->find(arg[9]);
	if (igroup_excluded == -1) error->all(FLERR,"Could not find exclude group ID for fix SkeleBreak command");
	groupbit_excluded = group->bitmask[igroup_excluded];
  }
  
  start_time = 0;
  if (narg == 10 && strcmp(arg[8],"start_time") == 0) {
	start_time = force->inumeric(FLERR,arg[9]);
  }
  
  int igroup_broken, igroup_partly, igroup_intact;
  group_added_flag = 0;
  if (narg == 12 && strcmp(arg[8],"added_group") == 0) {
	group_added_flag = 1;
	igroup_broken = group->find_or_create(arg[9]);
	groupbit_broken = group->bitmask[igroup_broken];
	inversebit_broken = group->inversemask[igroup_broken];
	igroup_partly = group->find_or_create(arg[10]);
	groupbit_partly = group->bitmask[igroup_partly];
	inversebit_partly = group->inversemask[igroup_partly];
	igroup_intact = group->find_or_create(arg[11]);
	groupbit_intact = group->bitmask[igroup_intact];
	inversebit_intact = group->inversemask[igroup_intact];
  }
  
  if (narg == 12 && strcmp(arg[8],"excluded_group") == 0 && strcmp(arg[10],"start_time") == 0) {
	group_excluded_flag = 1;
	igroup_excluded = group->find(arg[9]);
	if (igroup_excluded == -1) error->all(FLERR,"Could not find exclude group ID for fix SkeleBreak command");
	groupbit_excluded = group->bitmask[igroup_excluded];
	
	start_time = force->inumeric(FLERR,arg[11]);
  }
  
  group_excluded_flag = 0;
  if (narg == 14 && strcmp(arg[8],"excluded_group") == 0 && strcmp(arg[10],"added_group") == 0) {
	group_excluded_flag = 1;
	igroup_excluded = group->find(arg[9]);
	if (igroup_excluded == -1) error->all(FLERR,"Could not find exclude group ID for fix SkeleBreak command");
	groupbit_excluded = group->bitmask[igroup_excluded];
	...
	group_added_flag = 1;
	igroup_broken = group->find_or_create(arg[11]);
	groupbit_broken = group->bitmask[igroup_broken];
	inversebit_broken = group->inversemask[igroup_broken];
	igroup_partly = group->find_or_create(arg[12]);
	groupbit_partly = group->bitmask[igroup_partly];
	inversebit_partly = group->inversemask[igroup_partly];
	igroup_intact = group->find_or_create(arg[13]);
	groupbit_intact = group->bitmask[igroup_intact];
	inversebit_intact = group->inversemask[igroup_intact];
  }
  
  if (narg == 14 && strcmp(arg[8],"added_group") == 0 && strcmp(arg[12],"start_time") == 0) {
	group_added_flag = 1;
	igroup_broken = group->find_or_create(arg[9]);
	groupbit_broken = group->bitmask[igroup_broken];
	inversebit_broken = group->inversemask[igroup_broken];
	...
	start_time = force->inumeric(FLERR,arg[13]);
  }
  
  if (narg == 16 && strcmp(arg[8],"excluded_group") == 0 && strcmp(arg[10],"added_group") == 0 && strcmp(arg[14],"start_time") == 0) {
	group_excluded_flag = 1;
	igroup_excluded = group->find(arg[9]);
	if (igroup_excluded == -1) error->all(FLERR,"Could not find exclude group ID for fix SkeleBreak command");
	groupbit_excluded = group->bitmask[igroup_excluded];
	...
	start_time = force->inumeric(FLERR,arg[15]);
  }
  ...
  if (atom->molecular != 1)
    error->all(FLERR,"Cannot use fix SkeleBreak with non-molecular systems");
...
void FixSkeleBreak::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixSkeleBreak::setup(int vflag)
{
...
void FixSkeleBreak::post_integrate()
{
...
void FixSkeleBreak::check_ghosts()
{
  int i,j,n;
  tagint *slist;
...
void FixSkeleBreak::post_integrate_respa(int ilevel, int iloop)
{
...
int FixSkeleBreak::pack_forward_comm(int n, int *list, double *buf,
...
void FixSkeleBreak::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,last;
...
int FixSkeleBreak::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixSkeleBreak::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m,tmp1,tmp2;
...
void FixSkeleBreak::grow_arrays(int nmax)
{
...
void FixSkeleBreak::copy_arrays(int i, int j, int delflag)
{
...
int FixSkeleBreak::pack_exchange(int i, double *buf)
{
  int m = 0;
...
int FixSkeleBreak::unpack_exchange(int nlocal, double *buf)
{
  int m = 0;
...
double FixSkeleBreak::compute_vector(int n)
{
...
double FixSkeleBreak::memory_usage()
{
  int nmax = atom->nmax;
...
void FixSkeleBreak::print_copy(const char *str, tagint m, 
...
void FixSkeleBreak::set_arrays(int i)
{
  bond_count[i] = 0;
  // angle_count[i] = 0;
  // dihedral_count[i] = 0;
