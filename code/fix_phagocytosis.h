...
FixStyle(phagocytosis,FixPhagocytosis)

#else

#ifndef LMP_FIX_PHAGOCYTOSIS_H
#define LMP_FIX_PHAGOCYTOSIS_H

#include "fix.h"

namespace LAMMPS_NS {

class FixPhagocytosis : public Fix {
 public:
  FixPhagocytosis(class LAMMPS *, int, char **);
  ~FixPhagocytosis();
  int setmask();
  void init();
 ...
 private:
  ...
  int groupbit_target;
  int groupbit_protru;
  int inversebit_protru;
  double r_break;
  double r_breaksq;
  double r_protru_lo;
  double r_protru_losq;
  double r_protru_hi;
  double r_protru_hisq;
