...
FixStyle(addforce/sum,FixAddForceSum)

#else

#ifndef LMP_FIX_ADDFORCESUM_H
#define LMP_FIX_ADDFORCESUM_H

#include "fix.h"

namespace LAMMPS_NS {

class FixAddForceSum : public Fix {
 public:
  FixAddForceSum(class LAMMPS *, int, char **);
  ~FixAddForceSum();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  double xvalue,yvalue,zvalue,q_targ, xave, yave;
  /* "xave" and "yave" are the averages of several final stable values of xvalue and yvalue;
   by zunmin zhang
   */
