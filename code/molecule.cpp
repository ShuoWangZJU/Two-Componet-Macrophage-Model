...
void Molecule::read(int flag)
{
  ...
  while (1) {
    ...
    else if (strstr(line,"individual")) sscanf(line,"%d",&individual);
...
void Molecule::coords(char *line)
{
  ...
  for (int i = 0; i < natoms; i++) {
    ...
    sscanf(line,"%d %d %lg %lg %lg",&tmp,&type[i],&x[i][0],&x[i][1],&x[i][2]);
    x[i][0] *= sizescale;
    x[i][1] *= sizescale;
    x[i][2] *= sizescale;
    type[i] += toffset;
...
void Molecule::bonds(int flag, char *line)
{
  ...
  for (int i = 0; i < nbonds; i++) {
    ...
    if (individual)
      sscanf(line,"%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " %lg",
...
void Molecule::angles(int flag, char *line)
{
  ...
  for (int i = 0; i < nangles; i++) {
    ...
    if (individual)
      sscanf(line,"%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " %lg",
