...
FixStyle(protrusiveforce,FixProtrusiveForce)

#else

#ifndef LMP_FIX_PROTRUSIVEFORCE_H
#define LMP_FIX_PROTRUSIVEFORCE_H

#include "fix.h"

namespace LAMMPS_NS {

class FixProtrusiveForce : public Fix {
 public:
  FixProtrusiveForce(class LAMMPS *, int, char **);
  ~FixProtrusiveForce();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  double Fvalue, Fadd;
  int div_flag, Ndiv, finalstep, divstep;
