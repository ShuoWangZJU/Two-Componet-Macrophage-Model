...
FixStyle(skele/break/group,FixSkeleBreakGroup)

#else

#ifndef LMP_FIX_SKELE_BREAK_GROUP_H
#define LMP_FIX_SKELE_BREAK_GROUP_H

#include "fix.h"

namespace LAMMPS_NS {

class FixSkeleBreakGroup : public Fix {
 public:
  FixSkeleBreakGroup(class LAMMPS *, int, char **);
  ~FixSkeleBreakGroup();
  int setmask();
  void init();
 ...
 private:
  ...
  int break_bond_type;
  int band3_bond_type;
  int groupbit_break, groupbit_excluded, groupbit_broken, groupbit_partly, groupbit_intact, inversebit_broken, inversebit_partly, inversebit_intact;
  double r_break;
  double r_breaksq;
