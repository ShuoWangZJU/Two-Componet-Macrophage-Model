...
/* ---------------------------------------------------------------------- 
Syntax: fix ID group-ID polymer/activate style style_keywords Nevery itype jtype
  ...
  Fix(lmp, narg, arg)
{
  ...
  if (narg > 7 + arg_shift) error->all(FLERR,"Illegal fix polymer/activate command");
...
void FixPolymerActivate::post_integrate()
{
...
void FixPolymerActivate::post_integrate_respa(int ilevel, int iloop)
{
