...
void AngleAreaVolume::compute(int eflag, int vflag)
{
  ...
  if (update->ntimestep % 1000 == 0 && comm->me == 0) {
	for (m = 0; m < n_mol_max; m++) {
	  if (datt[m] > 0.0 && datt[m+n_mol_max] > 0.0) fprintf(stderr, "In angle area/volume, total area is %f, total volume is %f, desired is %f %f, in step %d\n", datt[m], datt[m+n_mol_max], a0[0], v0[0], update->ntimestep);
	}
  }
