...
FixStyle(filopodia/target,FixFilopodiaTarget)

#else

#ifndef LMP_FIX_FILOPODIA_TARGET_H
#define LMP_FIX_FILOPODIA_TARGET_H

#include "fix.h"

namespace LAMMPS_NS {

class FixFilopodiaTarget : public Fix {
 public:
  FixFilopodiaTarget(class LAMMPS *, int, char **);
  ~FixFilopodiaTarget();
  int setmask();
  void init();
 ...
 private:
  ...
  int igroup_target;
  int groupbit_target;
  int groupbit_side_ske;
  int inversebit_side_ske;
  double d_exclude;
  int *near_target_flag;
